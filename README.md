...
2. **`cirl` package**: This package provides all packages you read about above, and several other useful ones. You can use it on its own, or in conjunction with the `acmart`, `cvpr`, `iccv`, and `eccv` templates—which you must specify as a package option, so that the package loads necessary template packages on its own. You can read the comments in the package file for a full list of included packages and their intended use. Some examples include:
