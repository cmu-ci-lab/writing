...
3. **Latin abbreviations**: You can (and generally should) avoid using Latin abbreviations, as most of them have English equivalents that tend to make text read more naturally—which should be unsurprising, given that the rest of the text is written in English. In computer science, currently there is one exception where by convention we still need to use Latin, and I highlight this exception below—if you find yourself writing for a law journal, things will be different. If you prefer to use Latin abbreviations, you should typeset and use them correctly. Also, you should *not* italicize common Latin abbreviations.
	1. **et al.**: This stands for "et alia" (or "et alii"), which means "and others" in English. It has no period after the "et", but does have a period after "al.". This is the exception I mentioned above: you should use this when referring to two or more *additional* authors after the first author of a paper you are citing. See also **citations** below.
...
8. **both, either--or, colons**: When you use these words to fork the narration, you should place as much as possible of the shared sentence content before the fork. For example, you should say:
...
9. **that, who, which, commas**: Clauses that start with relative pronouns such as "that," "who," and "which" can be either restrictive or non-restrictive. Restrictive relative clauses provide information that is necessary to define the noun that precedes them. You should start such clauses with "that" and not separate them with commas. For example:
	> The paper that introduced path tracing has had transformative impact in graphics.

	Non-restrictive relative clauses provide additional information that is not necessary to define the noun that precedes them. You should start such clauses with "who" (for people) or "which" (otherwise), and separate them with commas. For example:
	> His latest paper, which introduced a new rendering algorithm, appeared in the Transactions on Graphics.

	As a rule of thumb, if omitting a relative clause would leave a sentence that stands on its own, the relative clause is non-restrictive.
...
11. **describe notation**: When you use notation in a sentence, you should include some short term explaining what the notation is, even if you have already defined it earlier in the document. For example, you should say:
...
12. **do not use diaereses**: Unless you are writing for The New Yorker, you generally do *not* need to use diaereses for words such as "naïve" (just write "naive"). The only exception is that you should use diaereses in names, for example "Zoë". If you insist on using a diaeresis on "naïve", then you should use diaereses consistently also in all sorts of other words, for example "coöperate" or "reëlect".

13. **use active voice**: You should prefer to write in active voice. For example, you should say:
...
14. **do not instruct the reader**: Instead of writing "See Section 3," you should write "We refer to Section 3." Instead of writing "Note that this is a tight bound," you should write "We note that this is a tight bound," or better yet "This is a tight bound" (you can almost always skip the words "(we) note that").

15. **use consistent nouns**: When you refer to some term several times throughout a sentence or paragraph, you should use consistent nouns. Consider this text:
...
16. **while, since**: Unless you want to provide time information—for example "While Yannis was writing, I was running experiments," and "I have been working on this project since I joined CMU"—you should replace these words with "even though" and "as" to help reduce ambiguities. For example, instead of:
	...
	you should write:
	> We can ignore the third-order term $d^3$, as we assume the distance $d$ to be small.

17. **footnotes, quotes, and punctuation**: Both footnotes and quotes should appear after punctuation.

18. **initialisms**: You should define all initialisms the first time you use them in your document, by fully typing out what they stand for. For example, you should say "the bidirectional reflectance distribution function (BRDF)" the first time you use "BRDF." I recommend against using initialisms in the abstract, which should be a succinct description of your work and thus should not introduce very specialized terminology. If you do need to use an initialism in the abstract, you should define it in its first appearance, and then define it once more the first time it appears in the main text. The reason is that the abstract and main text often appear separately from each other (for example, on web readers).
...
2. **AMSmath**: You should read the [AMSmath documentation](https://mirrors.ctan.org/macros/latex/required/amsmath/amsldoc.pdf) and the [Short Math Guide](https://mirrors.ctan.org/info/short-math-guide/short-math-guide.pdf) to get started. I very rarely use the standard `equation` environments anymore. My default environment is `align`, which basically acts as a drop-in superset replacement for `equation`. You should learn to use the `cases` environment.
...
10. **fractions**: Sometimes, you may get better results using the commands `\nicefrac` and `\sfrac`, from the packages `nicefrac` and `xfrac` respectively, instead of the basic command `\frac`. For example, when I type fractions in inline math mode, I almost always use `\nicefrac`. As another example, when I type a fraction whose numerator and denominator are themselves fractions, I use `\frac` for the main fraction, and `\sfrac` for its numerator and denominator. You should *not* use just `/` for fractions, especially not in inline math mode, as doing so can create ambiguities when there are multiple terms in the numerator or denominator.

11. **equality versus equivalence versus assignment**: You should know the difference between the symbols `=`, `\equiv` (renders $\equiv$), and `\coloneqq` (renders $\coloneqq$) and use them consistently. Conventions on how to use them differ. Within computer graphics, I suggest the following: You should use `\equiv` whenever you define something, which includes when you introduce notation. You should use `\coloneqq` for assignment, for example if you are describing the settings you choose (or assume) in some equation or algorithm in different use cases. You should use `=` everywhere else.
...
13. **big delimiters**: You should use properly sized delimiter pairs. Delimiters include parentheses, vertical bars, and square, curly, and angled brackets. You can have $\LaTeX$ choose the size of the delimiters automatically by using the `\left` and `\right` paired commands, for example: `\left(\frac{1}{2}\right)`, instead of `(\frac{1}{2})`. These must always be used in pairs and they do not work across line breaks. If your parenthetical block spans multiple lines, you must include an invisible `\right.` or `\left.` counterpart before the line break. Sometimes the automatic sizing is not ideal, in which case you can fine-tune with manual sizes: `\bigl`, `\Bigl`, `\biggl`, `\Biggl` (and the r counterparts). As these are static sizes, you do not need to take special care of multi-line equations. Additionally, using automatic sizing for inline equations can cause larger line spacing, which is undesirable for aesthetic reasons. The [mathdefs](#resources) package has a set of commands for delimiters that automatically take care of these issues and also prevent accidental unclosed delimiters (e.g., `\paren{}` for automatic sizing of parentheses, and `\sparen{}` for parentheses in an inline equation). You can use them as a replacement for standard delimiters.

14. **units**: You should learn how to use the `siunitx` package to correctly typeset units. You can read the [siunitx user manual](https://ctan.math.illinois.edu/macros/latex/contrib/siunitx/siunitx.pdf) to get started. In particular, units should be typeset in upright font, and with a space between them and the number they are quantifying. Using the command `\qty` from `siunitx` takes care of this and many other unit formatting issues (for example, correct typesetting of micro-modifiers).
...
1. **sizing**: Whenever possible, you should avoid excess white space on the sides of your figures and tables. You can size your graphics in $\LaTeX$ using the defined lengths `\textwidth` (for double-column figures and tables) or `\columnwidth` (for single-column). Ideally, the figure or table you are embedding should already have the correct size, as in the next item.

2. **fonts**: You should avoid figures and tables with rasterized text, which will inevitably look aliased and may even be impossible to read. For tables, you should never use rasterized images and instead should just input the table data as text. For figures, you should use vector graphics with all text typeset using appropriate fonts. You can create such figures using either `TikZ` (for example, with the [pgf](https://www.ctan.org/pkg/pgf) package), or using software such as Inkscape, Microsoft PowerPoint, or Adobe Illustrator.
	1. **font type and size**: The type and size of the font in your figures should match those in the captions of your document. In the `acmart` template, captions use the Linux Biolinum T font at 8pt size. In the `cvpr` and `iccv` templates, captions use the Nimbus Roman No9 L font (which is nearly identical to the Times New Roman font) at 9pt size. In the `eccv` template, captions use the Computer Modern Roman font (which is also known as CMU Serif) at 9pt size. In other templates, you can use the command `\expandafter\string\the\font` to find the font type and size.

	2. **figure and font sizing**: If you load figures with a `[width=...]` option, it will be impossible to exactly match the caption font size. Instead, in the software you use to create figures, you should set the canvas width to match the maximum width of single-column (`\linewidth`) and double-column (`\textwidth`) figures in your document, then set the figure's font size to match the caption font size as above. In the `acmart` template, `\linewidth` is 243.14749pt and `\textwidth` is 510.295pt. In the `cvpr` and `iccv` templates, `\linewidth` is 237.13594pt and `\textwidth` is 496.85625pt. In the `eccv` template, which is single-column, both `\linewidth` and `\textwidth` are 347.12361pt. In other templates, you can use the commands `\the\textwidth` and `\the\linewidth` to find these numbers.

3. **placement**: With the exception of wrap figures (see below), you should place figures and tables at either the top or the bottom of a page. I would suggest not having figures or tables at both the top and the bottom of the same column, sandwiching some text between them. Additionally, you should avoid pages that have predominantly figures and tables and only a small amount of text. In both cases, a reader may completely miss the text. If there is very little room for text, it is better to create a page with only figures and tables.

	Additionally, you should generally try to place figures and tables in the same page as, or at the very least near, the text that describes them. It can be difficult to predict where exactly a figure will end up while you are still writing your document. Once you finish writing, I recommend spending some time moving figures and tables around to optimize their placement relative to the text that describes them.
...
8. **table styling**: As a general rule of thumb, if you have vertical lines in your table, you are likely doing something wrong.
	...
	A standard table would look something like this example:
	~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ tex linenumbers
...
3. **bibliography**: Below are a few items to be careful about when creating your bibliography file.
	...
	3. **correct online bib entries before use**: Most digital libraries are notoriously bad at providing correct bibtex information. Always check when you cut-and-paste any online entry into your bib file. Common errors include:
		...
		2. DOI entries should include just the number, and not a full URL, for example:
			...
			and not
			~~~
			doi = {https://doi.acm.org/10.1145/1964179.1964185},
			~~~
	...
	4. **capitalization in reference titles**: You should not force capitalization by enclosing all your bib entry titles in double braces: `{{My Title}}`. *It is the bib style's job to decide on the capitalization of titles, not yours*, as this varies between publication venues. However, you do need to inform BibTeX when certain words must be capitalized, for instance if they are acronyms or proper names. Do this by enclosing just the capitalized letters in braces:
		...
		See the [TeX FAQ](https://texfaq.org/FAQ-capbibtex).
	...
	7. **know the publication history**: Many conferences have gone through a complex evolution of their publication history. For instance, EGSR was sometimes published as a book called "Rendering Techniques", but in 2008 it became a special issue of the Computer Graphics Forum journal. The proper way to cite a paper from EGSR therefore depends on the year. The same applies to SIGGRAPH, Eurographics, CVPR and ICCV. Nicolas Holzschuch summarizes the history of [SIGGRAPH](https://maverick.inria.fr/Membres/Nicolas.Holzschuch/siggraph.html), [EG](https://maverick.inria.fr/Membres/Nicolas.Holzschuch/eg.html), and [EGSR](https://maverick.inria.fr/Membres/Nicolas.Holzschuch/egsr.html) well.

### Other good practices

1. **PDF metadata**: You should use the [hyperref](https://ctan.org/pkg/hyperref) to set PDF metadata such as title, authors, topic, and keywords *for camera-ready PDF files*. (Never do this for PDF files under review, to avoid violating anonymity rules.) The latest version of the `acmart` template does that automatically. For the `cvpr`, `iccv`, and `eccv` templates, you can set this metadata as follows:
	~~~
...
2. **do not commit generated PDF and other files in git**: There are many files that are generated during building either $\LaTeX$ or code. Do not commit these files to the repository as it will cause conflicts for your collaborators. This includes the generated PDF file! Other common generated files in $\LaTeX$ include: `.aux, .bbl, .blg, .fdb_latexmk, .fls, .log, .out, .synctex.gz` files. See below about using a `.gitignore` file.

3. **project file structure**: You should have a separate `git` repository for each paper. Within the repository, I recommend structuring your files as follows. The [template repositories](#resources) are this structure, ready to clone, so you can skip building it by hand.

	1. **root, macro, and bib files:** Every paper should have these files. You should pick a short identifier for the paper (for example, "fermat" for the "Fermat paths" paper), then use it to name these three files with appropriate extensions (for example, `fermat.tex` for the root file, `fermat.sty` for the macro file, and `fermat.bib` for the bib file). The root file should include the preamble, title, your sections, and bibliography commands. The macro file should include any additional packages you want to include, and any macros you use for this paper (including macros for notation and commenting, see below). The bib file should include all bib entries.
...
4. **cross-references**: You will often need to reference different parts of a paper within the paper itself, such as sections, figures, tables, algorithms, list items, and so on.
	1. **labeling scheme**: You should choose a scheme for the labels of various types of references and stick to it consistently. I suggest using a 3-letter prefix: `sec:section-name`, `fig:figure-name`, `equ:equation-name`, `tab:table-name`, `alg:algorithm-name`, `enu:item-name`, `the:theorem-name`, `pro:proposition-name`, `def:definition-name`, `lem:lemma-name`, and so on.
	...
	5. **eqref**: You should use `\eqref` to reference equations, and `\ref` for everything else. Using `\eqref` ensures that your references match the labeling style `amsmath` uses to number equations. Alternatively, you should use `\cref` everywhere, as in the next item.
...
5. **inline lists**: If you want to create an inline list, you should enable the `inline` option of the [enumitem](https://ctan.org/pkg/enumitem?lang=en) package. You can then create a list using the `enumerate*` environment. For example,
	~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ tex linenumbers
...
3. **`colordefs` package**: This package provides categorical color palettes to use throughout your document, including with other packages in this repository. Each palette provides four colors, each at three levels. The palettes come from published color schemes chosen to stay legible for readers with color vision deficiencies and to survive grayscale printing; the package file records how each one scores on both counts, so you can pick with your eyes and check the numbers afterwards.
...
5. **`cirl` package**: This package provides all packages you read about above, and several other useful ones. You can use it on its own, or in conjunction with the `acmart`, `cvpr`, `iccv`, and `eccv` templates—which you must specify as package options, alongside any other options, so that the package loads any necessary template packages on its own. You can read the comments in the package file for a full list of included packages and their intended use. Some examples include:
	1. **`microtype`**: The [microtype](https://ctan.org/pkg/microtype) package incorporates several micro adjustments to make the typography easier to read and more beautiful. As an added bonus, it also tends to shorten the paper slightly due to differences in line wrapping.
	2. **`xspace`**: The [xspace](https://www.ctan.org/pkg/xspace) package allows you to define macros that interact properly with surrounding spaces and punctuation.

6. **`Makefile`**: You can use this file with `make` to build a document from the command line (assuming you have TeX installed), as well as clean files $\LaTeX$ produces during compilation. You should not need to edit it: it reads the target file for your project from a `config.mk` file beside it, so that the `Makefile` itself stays a verbatim copy of the one here. Write one line in `config.mk` naming the basename of your root `.tex` file.
	~~~
	TARGET = fermat
	~~~
	You can list several targets, separated by spaces, to build a paper and its supplement together. Without a `config.mk`, the `Makefile` builds `paper.tex`.
...
Each of the following template repositories is a ready-made project that conforms with the above suggestions, includes the packages and utilities at its root, and is kept in sync with the main writing repository.
- <https://github.com/cmu-ci-lab/paper-tog-template> (`acmart`, for SIGGRAPH, SIGGRAPH Asia, and TOG)
- <https://github.com/cmu-ci-lab/paper-cvpr-template> (for CVPR and ICCV)
- <https://github.com/cmu-ci-lab/paper-eccv-template> (for ECCV)
...
2. **updating files**: Each template repository includes venue-specific files, such as the `acmart` template, the `cvpr`, `iccv`, and `eccv` style files, and bibliography style files. These files typically change from year to year. We try to keep the venue-specific files of each template up to date. But nevertheless, you should always download the latest author kit from the venue, and put its `.sty`, `.cls`, and `.bst` files in your project directory.

## Reading

If you were to read just one book on writing, I would recommend [On Writing Well](https://www.amazon.com/Writing-Well-Classic-Guide-Nonfiction/dp/0060891548), by William Zinsser. If you are in my lab, I would be happy to buy you a copy if you cannot get your hands on one.
...
- <https://www.slideshare.net/jdily/how-to-write-a-siggraph-paper>
